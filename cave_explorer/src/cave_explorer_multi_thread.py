...
class CaveExplorer:
    ...
    def identify_frontiers(self, current_map):
            ...
            if (self._frontier_cache is not None and 
                self._last_map_stamp == current_map.header.stamp):
                return self._frontier_cache

            dims = self.get_map_dimensions()
            if not dims:
                return []
            ...
            # Split the map into chunks for parallel processing
            chunk_height = height // mp.cpu_count()
            chunks = []
            
            for i in range(0, height, chunk_height):
                end_i = min(i + chunk_height, height)
                # Include one row overlap at boundaries
                start_i = max(0, i - 1) if i > 0 else i
                chunks.append((map_array[start_i:end_i + 1], start_i))

            # Process chunks in parallel
            process_chunk_partial = partial(self._process_chunk, width=width)
            results = self.pool.map(process_chunk_partial, chunks)

            # Combine results and remove duplicates
            frontiers = set()
            for chunk_frontiers in results:
                frontiers.update(chunk_frontiers)

            self._frontier_cache = list(frontiers)
            return self._frontier_cache
